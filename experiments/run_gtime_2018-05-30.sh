...
# for buf_size in 1 5;do
    ...
    # for input in ~/Data/timedPatternMatching/Experiment3_AT-1000.tsv; do
        ...
        for t in $(seq $RUN_TIMES); do
            /usr/local/bin/gtime -v -o log/$date_str/filt-monaa-time-accel-${buf_size}-${input##*/}-$t.log bash -c "../build/filt -atf ./dots/accel_filter.dot < /tmp/monaa.input | ../../monaa/build/monaa -f ./dots/accel.dot" > /dev/null
        done
    done
done
...
# for input in ~/Data/timedPatternMatching/torque-1000.dat; do
    ...
    ../build/filt -atf ./dots/torque_filter.dot < /tmp/monaa.input > /tmp/torque.filtered
    ../../monaa/build/monaa -Df ./dots/torque.dot < /tmp/monaa.input | ./calc_unused/get_intervals.sh > /tmp/torque.intervals
    ./calc_unused/calc_unused /tmp/torque.filtered /tmp/torque.intervals > log/$date_str/unused_rate-torque-10-${input##*/}-$t.log
done
...
#for input in ~/Data/timedPatternMatching/Experiment3_AT-1000-gear-signal.dat; do
    ...
    ../build/filt -atf ./dots/gear_filter.dot < /tmp/monaa.input > /tmp/gear.filtered
    ../../monaa/build/monaa -Df ./dots/gear.dot < /tmp/monaa.input | ./calc_unused/get_intervals.sh > /tmp/gear.intervals
    ./calc_unused/calc_unused /tmp/gear.filtered /tmp/gear.intervals > log/$date_str/unused_rate-gear-10-${input##*/}-$t.log
done
...
for input in $ACCEL_DIR/Experiment3_AT-*.tsv; do
    ...
    for t in $(seq $RUN_TIMES); do
        /usr/local/bin/gtime -v -o log/$date_str/no-filt-monaa-time-accel-${buf_size}-${input##*/}-$t.log bash -c "cat /tmp/monaa.input | ../../monaa/build/monaa -f ./dots/accel.dot" > /dev/null
    done

    ../build/filt -atf ./dots/accel_filter.dot < /tmp/monaa.input > /tmp/accel.filtered
    ../../monaa/build/monaa -f ./dots/accel.dot < /tmp/monaa.input | ./calc_unused/get_intervals.sh > /tmp/accel.intervals
    ./calc_unused/calc_unused /tmp/accel.filtered /tmp/accel.intervals > log/$date_str/unused_rate-accel-10-${input##*/}-$t.log
done
