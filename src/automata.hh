...
struct NFAState {
  ...
  NFAState () : isMatch(false), next({}) {}
  NFAState (bool isMatch) : isMatch(isMatch), next({}) {}
  NFAState (bool isMatch, std::unordered_map<unsigned char, std::vector<std::weak_ptr<NFAState>>> next) : isMatch(isMatch), next(next) {}
};
...
struct NFA : public Automaton<NFAState> {
