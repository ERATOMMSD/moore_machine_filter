...
template<class T>
void convBoostTA(const BoostTimedAutomaton &, T &);
...
void convBoostTA(const BoostTimedAutomaton &BoostTA, TimedAutomaton &TA)
{
  ...
  for (auto first = vertex_range.first, last = vertex_range.second; first != last; ++first) {
    ...
    for (auto firstEdge = edge_range.first, lastEdge = edge_range.second; firstEdge != lastEdge; ++firstEdge) {    
      ...
      stateConvMap[*first]->nextMap[boost::get(&BoostTATransition::c, BoostTA, *firstEdge)].emplace_back(std::move(transition));
    }
  }
}

template<>
void convBoostTA(const BoostTimedAutomaton &BoostTA, NFA &TA)
{
  TA.states.clear();
  TA.initialStates.clear();
  auto vertex_range = boost::vertices(BoostTA);
  ...
  for (auto first = vertex_range.first, last = vertex_range.second; first != last; ++first) {
    auto edge_range = boost::out_edges(*first, BoostTA);
    for (auto firstEdge = edge_range.first, lastEdge = edge_range.second; firstEdge != lastEdge; ++firstEdge) {    
      stateConvMap[*first]->nextMap[boost::get(&BoostTATransition::c, BoostTA, *firstEdge)].emplace_back(stateConvMap[boost::target(*firstEdge, BoostTA)]);
    }
  }
}
